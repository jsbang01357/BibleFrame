...
PASSAGE_MAX_BYTES = 2_500


def split_utf8(text: str, max_bytes: int) -> list[str]:
    """UTF-8 바이트 한도를 넘지 않게 문장, 마지막에는 글자 경계로 나눈다."""
...
def passage_groups(items: list[dict[str, object]], max_bytes: int = PASSAGE_MAX_BYTES) -> list[list[tuple[int, str]]]:
    """장 안에서 절 경계를 보존한 검색용 소청크를 만든다."""
    rows: list[tuple[int, str]] = []
    for item in items:
        verse = int(item["verse"])
        parts = split_utf8(str(item["text"]), max_bytes - 32)
        rows.extend((verse, f"{verse} {part}" if index == 0 else f"{verse}절 이어짐 {part}") for index, part in enumerate(parts))

    groups: list[list[tuple[int, str]]] = []
    current: list[tuple[int, str]] = []
    for row in rows:
        candidate = "\n".join(text for _, text in [*current, row])
        if current and len(candidate.encode("utf-8")) > max_bytes:
...
def build() -> dict[str, int]:
    ...
    passages_path = rag / "haystack_passages.jsonl"
    with (
        rag_path.open("w", encoding="utf-8") as stream,
        haystack_path.open("w", encoding="utf-8") as haystack_stream,
        passages_path.open("w", encoding="utf-8") as passages_stream,
    ):
        for (code, chapter), items in chapters.items():
            ...
            for index, passage in enumerate(passage_groups(items), 1):
                passage_record = {
                    "id": f'{record["id"]}-p{index:02d}',
                    "content": "\n".join(text for _, text in passage),
                    "meta": {
                        **record["metadata"],
                        "chunk_index": index,
                        "verse_start": passage[0][0],
                        "verse_end": passage[-1][0],
                    },
                }
                passages_stream.write(json.dumps(passage_record, ensure_ascii=False, separators=(",", ":")) + "\n")
...
"""
    ...
    with zipfile.ZipFile(downloads / "bibleframe-rag.zip", "w", zipfile.ZIP_DEFLATED) as archive:
        for name, content in (
            ...
            ("haystack_passages.jsonl", passages_path.read_bytes()),
