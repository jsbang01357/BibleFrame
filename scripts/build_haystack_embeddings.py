#!/usr/bin/env python3
"""Vertex 다국어 임베딩을 재개 가능하게 생성해 Haystack 문서에 연결한다."""
...
def main() -> None:
    ...
    completed = {str(item["id"]): item for item in [*load_lines(args.output), *load_lines(args.checkpoint)]}
            ...
            for attempt in range(args.retries):
                try:
                    embeddings = client.embed_many(
                        [str(item["content"]) for item in batch], "RETRIEVAL_DOCUMENT",
                    )
                    break
